<?php

require_once('Dog.php');
require_once('Fish.php');
require_once('Cat.php');
require_once('Parrot.php');
require_once('FarmAnimalCharctFactory');
require_once('JungleAnimalCharctFactory');

abstract class AnimalFactory {

    protected $animal;

    public function getCharct(String $type){
        if ($type == 'Farm') {
            return new FarmAnimalCharctFactory();
        } 
        
        return new JungleAnimalCharctFactory();
    }

    public function createAnimal(String $type, String $specie){
        if ($specie == 'Dog') {
            $this->animal = new Dog( getCharct($type) );
        } 
        else if ($specie == 'Cat') {
            $this->animal = new Cat( getCharct($type) );
        }
        else if ($specie == 'Fish') {
            $this->animal = new Fish( getCharct($type) );
        }
        else if ($specie == 'Parrot') {
            $this->animal = new Parrot( getCharct($type) );
        }
    }

    public function bringAnimal(){
        return $this->animal;
    }

} 

?>